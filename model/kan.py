...
class KAN(nn.Module):
    def __init__(
        self,
        input_dim,
        hidden_dims=[16, 8], # 隐藏层结构
        output_dim=1,        # RUL 预测通常为 1
        grid_size=5,
        spline_order=3,
        scale_noise=0.1,
        scale_base=1.0,
    ...
    def forward(self, x: torch.Tensor, update_grid=False):
        ...
        Args:
            x: 输入特征
            update_grid: 是否更新网格 (训练初期可选 True，通常 False)
        Returns:
            x: 预测值 [batch_size, output_dim]
        """
        # 通过隐藏层
        ...
        # 通过输出层
        if update_grid:
            self.output_layer.update_grid(x)
    ...
    def regularization_loss(self, regularize_activation=1.0, regularize_entropy=1.0):
        """
        (可选) 计算正则化损失。
        在使用 MSELoss 训练时，您可以手动将此项加到 loss 中以利用 KAN 的稀疏特性。
        """
        loss = sum(
            layer.regularization_loss(regularize_activation, regularize_entropy)
            for layer in self.layers
        )
        loss += self.output_layer.regularization_loss(regularize_activation, regularize_entropy)
        return loss
    ...
    def get_feature_importance(self, x):
        ...
        # 对 out_features 和 grid 维度求均值，保留 in_features 维度
        importance = first_layer.spline_weight.abs().mean(dim=(0, 2))
        
        if first_layer.enable_standalone_scale_spline:
            scaler_imp = first_layer.spline_scaler.abs().mean(dim=0)
            importance = importance * scaler_imp
